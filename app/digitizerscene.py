...
from .var_classes import Instructions
from .itemsGrahpicScene import RectangleAnnotation, PointAnnotation, PolygonAnnotation
	...
	def mousePressEvent(self, event):
		...
		if self.image_item in current_item:
			...
			if self.current_instruction == Instructions.Move_Instruction and event.button() == QtCore.Qt.RightButton:
				...
				if not self.instruction_active and len(current_item) >= 1:
				...
				# finish move instruction
					...
					coordinates = self.active_item.get_coords()
				...
				if not self.instruction_active:
					...
					for item in self.items():
						if hasattr(item, 'object_type'):
							...
							found_near_geometry, idx_point = item.close_point(event.scenePos(), 40)
			...
			# Continue with Polygon objects
				...
				if self.current_instruction == Instructions.Rectangle_Instruction:
					...
					if event.button() == QtCore.Qt.RightButton:
						...
						coordinates = self.rectangle_item.get_coords()
